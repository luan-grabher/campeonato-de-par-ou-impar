...
function createId(prefix) {
  ...
  createId.fallbackCounter = (createId.fallbackCounter || 0) + 1;
  return `${prefix}-${Date.now()}-${createId.fallbackCounter}`;
}

export function createLocalEmail(name) {
  const trimmedName = (name || '').trim().toLowerCase();
  const normalized = trimmedName.replace(/[^a-z0-9]+/g, '.').replace(/^\.+|\.+$/g, '');
...
function buildUpdatedUser(user, { won, playerNumber, opponentElo, createdAt }) {
  ...
  // Standard Elo expectation formula: returns a 0..1 win probability for the player.
  // E = 1 / (1 + 10^((Ropponent - Rplayer) / 400)).
  const expectedScore = 1 / (1 + 10 ** ((opponentElo - previousElo) / ELO_RATING_DIVISOR));
  const score = won ? 1 : 0;
...
function generateRoomCode(existingRooms) {
  ...
  const rejectionLimit = Math.floor(256 / alphabetSize) * alphabetSize;
  const byte = new Uint8Array(1);
  let code = '';

  for (let attempt = 0; attempt < MAX_ROOM_CODE_ATTEMPTS; attempt += 1) {
    code = '';
    // Rejection sampling keeps each room-code character uniformly distributed.
    while (code.length < codeLength) {
      crypto.getRandomValues(byte);
      if (byte[0] < rejectionLimit) {
        code += ROOM_CODE_ALPHABET[byte[0] % alphabetSize];
      }
    }

    if (!existingRooms.some((room) => room.code === code)) {
      return code;
    }
  }
...
export function recordQuickMatch({
  ...
  return updateState((state) => {
    ...
    return {
      ...state,
      users,
      matches: [match, ...state.matches].slice(0, MAX_MATCH_HISTORY),
    };
  });
}
